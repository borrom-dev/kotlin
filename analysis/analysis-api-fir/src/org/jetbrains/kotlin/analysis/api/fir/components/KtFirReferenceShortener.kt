...
private class ElementsToShortenCollector(
    ...
    private fun FirScope.correspondingClassIdIfExists() = when (this) {
     ...
     */
    private fun FirScope.isScopeForClassCloserThanAnotherScopeForClass(another: FirScope, from: KtElement): Boolean {
        ...
        val classId = correspondingClassIdIfExists()
            ?: error("FirScope is expected to be one of FirNestedClassifierScope and FirClassUseSiteMemberScope")
        val classIdOfAnother = another.correspondingClassIdIfExists()
            ?: error("Another FirScope is expected to be one of FirNestedClassifierScope and FirClassUseSiteMemberScope")
        ...
        // than `another`.
        val closestClassId = from.containingClass()?.let { containingClass ->
            findMostInnerClassMatchingId(containingClass, buildSet {
                add(classId)
                add(classIdOfAnother)
                classId.idWithoutCompanion()?.let { add(it) }
                classIdOfAnother.idWithoutCompanion()?.let { add(it) }
            })
        } ?: return false
    ...
    private fun findMostInnerClassMatchingId(innerClass: KtClass, candidates: Set<ClassId>): ClassId? {
        ...
        while (classInNestedClass != null) {
            ...
            val matchingClassId = candidates.singleOrNull { it == containingClassId }
            if (matchingClassId != null) return matchingClassId
     ...
     *
     * Example:
     *   import ... // scope1  FirExplicitSimpleImportingScope
     *   // scope2  FirPackageMemberScope
     *   class Outer { // scope3  FirClassUseSiteMemberScope
     *     class Inner { // scope4  FirClassUseSiteMemberScope or FirNestedClassifierScope
     *       fun foo() { // scope5  FirLocalScope
     *
     ...
     *
     *       }
     *       companion object { // scope6  FirClassUseSiteMemberScope or FirNestedClassifierScope
     *       }
     *     }
     *   }
     */
    private fun List<FirScope>.hasScopeCloserThan(base: FirScope, from: KtElement) = when (base) {
        is FirLocalScope -> false

        is FirClassUseSiteMemberScope -> any { scope ->
            if (scope is FirLocalScope) return true
            if (!scope.isScopeForClass()) return@any false
            scope.isScopeForClassCloserThanAnotherScopeForClass(base, from)
        }

        is FirNestedClassifierScope -> any { scope ->
            if (scope is FirLocalScope) return true
            if (!scope.isScopeForClass()) return@any false
            scope.isScopeForClassCloserThanAnotherScopeForClass(base, from)
        }

        is FirExplicitSimpleImportingScope -> any {
            it is FirLocalScope || it is FirClassUseSiteMemberScope || it is FirNestedClassifierScope
        }

        is FirPackageMemberScope -> any {
            it is FirLocalScope || it is FirClassUseSiteMemberScope || it is FirNestedClassifierScope || it is FirExplicitSimpleImportingScope
        }

        else -> any {
            it is FirLocalScope || it is FirClassUseSiteMemberScope || it is FirPackageMemberScope
        }
    }
    ...
    private fun PsiFile.hasImportDirectiveForDifferentSymbolWithSameName(classId: ClassId): Boolean {
        ...
        return importDirectivesWithSameImportedFqName.isNotEmpty() &&
                importDirectivesWithSameImportedFqName.all { it.importedFqName?.asString() != classId.asFqNameString() }
    }

    private fun shortenClassifierIfAlreadyImported(
        classId: ClassId, element: KtElement, classSymbol: FirClassLikeSymbol<*>, positionScopes: List<FirScope>
    ): Boolean {
        ...
        val scopeForClass = matchingAvailableSymbol?.scope
        if (scopeForClass != null) {
            ...
            val qualifiedAccessCandidates = findCandidateForPropertyAccess(classSymbol.annotations, typeArguments, name, element)
            if (qualifiedAccessCandidates.mapNotNull { it.candidate.originScope }
                    .hasScopeCloserThan(scopeForClass, element)) return false
    ...
    private fun findCandidateForQualifiedAccess(
        ...
        val functionCall = firQualifiedAccess as? FirFunctionCall
        val fakeFirQualifiedAccess: FirQualifiedAccess?
        if (functionCall == null) {
            fakeFirQualifiedAccess = buildPropertyAccessExpression {
                annotations.addAll(firQualifiedAccess.annotations)
                typeArguments.addAll(firQualifiedAccess.typeArguments)
        ...
        // Since we create a fake FIR and try to collect it using resolver, it can cause exception when it cannot resolve it.
        val candidates = try {
            AllCandidatesResolver(shorteningContext.analysisSession.useSiteSession).getAllCandidates(
                firResolveSession, fakeFirQualifiedAccess, fakeCalleeReference.name, expressionInScope
            )
    ...
    private fun findCandidateForPropertyAccess(
        annotations: List<FirAnnotation>,
        typeArguments: List<FirTypeProjection>,
        name: Name,
        expressionInScope: KtElement,
    ): List<OverloadCandidate> {
        val fakeCalleeReference = buildSimpleNamedReference { this.name = name }
    ...
    private fun List<OverloadCandidate>.findScopeForSymbol(symbol: FirBasedSymbol<*>) = firstOrNull {
     ...
     *        findCandidateForQualifiedAccess() above.
    ...
    private fun shortenIfAlreadyImported(
        ...
        val candidates = findCandidateForQualifiedAccess(firQualifiedAccess, calledSymbol.name, expressionInScope)
    ...
    private fun processPropertyReference(resolvedNamedReference: FirResolvedNamedReference) {
        ...
        if (option != ShortenOption.DO_NOT_SHORTEN) {
            ...
            if (shortenIfAlreadyImported(firPropertyAccess, callableSymbol, referenceExpression)) {
        ...
        processCallableQualifiedAccess(
            ...
            qualifiedProperty,
            qualifiedProperty,
            shorteningContext::findPropertiesInScopes
        )
    }
    ...
    private fun processFunctionCall(functionCall: FirFunctionCall) {
        ...
        if (option != ShortenOption.DO_NOT_SHORTEN) {
            if (shortenIfAlreadyImported(functionCall, calledSymbol, callExpression)) {
        ...
        processCallableQualifiedAccess(
            ...
            callExpression,
            shorteningContext::findFunctionsInScopes
        )
    }
