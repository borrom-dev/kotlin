...
fun SessionHolder.collectTowerDataElementsForClass(owner: FirClass, defaultType: ConeKotlinType): TowerElementsForClass {
    ...
    for (superType in lookupSuperTypes(owner, lookupInterfaces = false, deep = true, useSiteSession = session, substituteTypes = true)) {
        ...
        superClass.staticScope(this)
            ...
            ?.asClassBasedTowerDataElement(ownerLookupTag = superClass.symbol.toLookupTag())
...
class FirTowerDataElement(
    ...
    val isLocal: Boolean,
    val ownerLookupTag: ConeClassLikeLookupTag? = null
) {
...
fun FirScope.asClassBasedTowerDataElement(ownerLookupTag: ConeClassLikeLookupTag?): FirTowerDataElement =
    FirTowerDataElement(scope = this, implicitReceiver = null, isLocal = false, ownerLookupTag = ownerLookupTag)
