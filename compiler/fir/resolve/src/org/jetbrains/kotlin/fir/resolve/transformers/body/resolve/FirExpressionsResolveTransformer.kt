...
open class FirExpressionsResolveTransformer(transformer: FirAbstractBodyResolveTransformerDispatcher) :
        ...
        var result = when (val callee = qualifiedAccessExpression.calleeReference) {
            ...
            is FirErrorNamedReference -> {
                if (qualifiedAccessExpression.typeRef !is FirResolvedTypeRef) {
                    storeTypeFromCallee(qualifiedAccessExpression, false)
                }
                qualifiedAccessExpression
            }
            else -> {
    ...
    private fun FirTypeOperatorCall.transformTypeOperatorCallChildren(data: ResolutionMode): FirTypeOperatorCall {
        ...
        return transformOtherChildren(transformer, data)
    }
    ...
    private fun storeTypeFromCallee(expression: FirDesugaredAssignmentValueReferenceExpression) {
        ...
        if (referencedExpression.typeRef is FirImplicitTypeRef) {
            referencedExpression.transform<FirExpression, _>(transformer, ResolutionMode.ContextIndependent)
        }
