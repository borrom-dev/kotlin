...
object NiceContractSyntaxFunctionChecker : FirSimpleFunctionChecker() {
    override fun check(declaration: FirSimpleFunction, context: CheckerContext, reporter: DiagnosticReporter) {
        checkFeatureIsEnabled(declaration, context, reporter)
    }
}

object NiceContractSyntaxPropertyChecker : FirPropertyChecker() {
...
private fun checkFeatureIsEnabled(declaration: FirContractDescriptionOwner, context: CheckerContext, reporter: DiagnosticReporter) {
    ...
    if (!languageVersionSettings.supportsFeature(LanguageFeature.NiceContractSyntax)) {
        reporter.reportOn(source, FirErrors.UNSUPPORTED_FEATURE, LanguageFeature.NiceContractSyntax to languageVersionSettings, context)
    }
}
